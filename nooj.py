...
def check_inflected_forms(inflected_forms):
    ...
    for inflected_forms in sorted(lemmas.values(), key= lambda x: x[0]["group"]):
        ...
        for mood in ["indicatif", "subjonctif"]:
            for voice in ["actif", "passif"]:
                for tense in ["présent", "imparfait", "futur", "parfait", "plus-que-parfait", "futur antérieur"]:
                    for person in [1, 2, 3, 4, 5, 6]:
                        key = f"group={group}, mood={mood}, voice={voice}, tense={tense}, person={person}"
                        
                        if lemma == "timere" and voice == "passif" and (tense == "parfait" or tense =="plus-que-parfait" or tense == "futur antérieur"):
                                continue
                        
                        exist = False
                        for inflected_form in inflected_forms:
                            if inflected_form["person"] == person and inflected_form["mood"] == mood and inflected_form["voice"] == voice and inflected_form["tense"] == tense:
                                exist = True

                        if mood == "subjonctif" and (tense == "futur" or tense == "futur antérieur"):
                            if exist:
    ...
    if len(missing) or len(extras):
        ...
        for key, lemmas in missing.items():
            ...
            raise Exception(f"{len(missing)} missing forms and {len(extras)} extra forms")
