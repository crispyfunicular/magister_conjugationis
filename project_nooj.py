...
def get_verbs(verbs_latin_json: str):
    """ Gets a dic generated by NooJ
    And processes it through pynooj In order to get list of dicts

    *Input* (one)
    verbs_latin_csv = str (path to a NooJ dic)

    *Output* (one)
    retlist_verbs = list of dicts (one per line)
    """

    with open (verbs_latin_json, "r", encoding="utf-8") as f:
        return json.load(f)
